...
class PeerType(object):
    ...
    def ip(self, ip):
        ...
        :type: str
        """
        if ip is not None and not re.search('^(\\d{1,3}\\.){3}\\d{1,3}$', ip):  # noqa: E501
            raise ValueError("Invalid value for `ip`, must be a follow pattern or equal to `/^(\\d{1,3}\\.){3}\\d{1,3}$/`")  # noqa: E501
